...
export default function ProductDetail() {
    ...
    const {navigate, products ,currency, addToCard }=useAppContext();
    const {id}= useParams();
    const [relatedProducts, setRelatedproducts] = React.useState([]);
    const product = products.find((item)=>item._id===id);
    const [thumbnail, setThumbnail] = React.useState(product?.image[0]?product.image[0]:null);
    useEffect(()=>{
        if(products.length > 0){
            let copyProducts = products.slice()
            copyProducts =copyProducts.filter((item)=>product.category=== item.category)
